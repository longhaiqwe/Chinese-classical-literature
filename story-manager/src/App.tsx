...
import StoryDeleter from './components/StoryDeleter';
...
function App() {
  ...
  const [viewMode, setViewMode] = useState<'generator' | 'deleter'>('generator');
  ...
  return (
    <div className="min-h-screen bg-paper-50 font-serif p-8">
      ...
      {viewMode === 'deleter' ? (
        <StoryDeleter onBack={() => setViewMode('generator')} />
      ) : (
